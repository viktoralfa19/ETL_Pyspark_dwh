...
class ETLGrupoCombustibleBL():
    """Lógica de negocio para el ingreso de datos en la dimensión de grupos de combustibles"""
    ...
    def Elt_main(self):
        try:
            ...
            print('---- Proceso de ETL de Dimensiones de Grupos de Combustiblen ---- \n')
            print('DATAWAREHOUSE: dwh_sirio')
            print('DIMENSIÓN: dim_grupo_combustible \n')
            ...
            self.Load_data(transform_data,'cen_dws.dim_grupo_combustible')
    ...
    def Extract_data_agentes (self):
        ...
        grupos = self._genericDataFrame.GetDataHdfs('CFG_GrupoCombustible','file_CFG_GrupoCombustible')
        
        grupos = grupos\
        .select(grupos.CodigoGrupoCombustible.alias('grcomb_id_bk'),
                upper(grupos.NombreGrupoCombustible).alias('grcomb_nombre'))
    ...
    def Transform_data (self, extract_data):
        ...
        maxPk = self._accesoDatos.GetMaxPkTable('SELECT COALESCE(MAX(grcomb_id_pk),0) pk FROM cen_dws.dim_grupo_combustible')   
        self.catalogoDW = self._accesoDatos.GetAllData('cen_dws.dim_grupo_combustible')
        fecha_carga = datetime.datetime.today()

        for toReplace, replacement in zip(paramReplace, valueReplace):
            extract_data = extract_data\
            .select(extract_data.grcomb_id_bk,
                    Funct.trim(regexp_replace(extract_data.grcomb_nombre, toReplace, replacement)).alias('grcomb_nombre'))
        ...
        if self.catalogoDW.count()==0: 
            catalogos = extract_data.select((maxPk+row_number().over(Window.partitionBy()\
                            .orderBy(extract_data.grcomb_id_bk))).alias('grcomb_id_pk'),'*', 
            ...
            data_to_compare = extract_data.join(self.catalogoDW,
                                                (extract_data.grcomb_id_bk==self.catalogoDW.grcomb_id_bk), how='left')\
                                        .select(Funct.when(self.catalogoDW.grcomb_id_pk.isNull(),0)\
                                                .otherwise(self.catalogoDW.grcomb_id_pk).alias("grcomb_id_pk"), 
                                                extract_data.grcomb_id_bk,extract_data.grcomb_nombre)                

            # Identificación de registros nuevos que deben ser insertados en el DWH
            dataNw = data_to_compare.filter(data_to_compare.grcomb_id_pk==0).drop('grcomb_id_pk')

            catalogos = dataNw.select((maxPk+row_number().over(Window.partitionBy()\
                                                               .orderBy(dataNw.grcomb_id_bk))).alias('grcomb_id_pk'),
                                      '*',Funct.concat(Funct.lit(fecha_carga)).cast('timestamp').alias('fecha_carga'))

            # Identificación de registros que han sufrido cambios en la base transaccional y deben ser modificados en el DW
            dataMdf = data_to_compare.filter(data_to_compare.grcomb_id_pk!=0)
    ...
    def Load_Upsert_data(self,transform_data):
        ...
        result_transact = []
        transform_data_map = transform_data.rdd.map(lambda x: (x.grcomb_id_pk, [x.grcomb_id_bk, x.grcomb_nombre,x.fecha_carga]))
        
        querys_data_insert = transform_data_map.map(lambda x: Queries.Upsert_Query_Dim_Grupo_Combustible(x))
