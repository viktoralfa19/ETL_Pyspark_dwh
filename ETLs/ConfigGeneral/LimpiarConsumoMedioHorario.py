...
class LimpiarConsumoMedioHorario():
    """Clase que permite realizar la extracción, limpieza y validación de datos de Consumo Medio Horario de SIVO."""
    ...
    def Elt_main(fecha_inicio,fecha_fin,modulo,deteleTmpQuery):
        try:
            ...
            print('---- Proceso de ETL de Hechos de Consumo Medio Horario ---- \n')
            print('DATAWAREHOUSE: dwh_sirio')
            print('DIMENSIÓN: fact_demanda_medio_horaria \n')
            ...
            transform_data = LimpiarConsumoMedioHorario.Extract_Transform_data(fecha_inicio,accesoDatos)
            
            print('2. Cargar  datos\n')
            LimpiarConsumoMedioHorario.Load_data(transform_data,'cen_dws.fact_demanda_medio_horaria',accesoDatos)
    ...
    def Extract_Transform_data (fecha_inicio,accesoDatos):
        ...
        consumos = LimpiarConsumoMedioHorario\
        ...
        .select(regexp_replace(substring(consumos.FechaHora.cast('string'),0,10),'-','').cast('int').alias('tmpo_id_fk'),
                ...
                consumos.MV_Validado.cast('float').alias('demmh_potencia')
               ).withColumn('fecha_carga',lit(fecha_actual))
        
        faltantes = consumos.filter((consumos.agt_id_fk.isNull()) | (consumos.tmpo_id_fk.isNull()) |\
                                    (consumos.hora_id_fk.isNull()) | (consumos.eldem_id_fk.isNull()) |\
                                    (consumos.compdem_id_fk.isNull()))
        #faltantes.show()
