...
class ETLInterconexionBL():
    """Lógica de negocio para el ingreso de datos en la dimensión de Agentes de Interconexión"""
    ...
    def Elt_main(self):
        try:
            ...
            print('---- Proceso de ETL de Dimensiones de AGENTE de INTERCONEXIÖN ---- \n')
            print('DATAWAREHOUSE: dwh_sirio')
            print('DIMENSIÓN: dim_agt_interconexion \n')
            ...
            self.Load_data(transform_data,'cen_dws.dim_agt_interconexion')
    ...
    def Extract_data_agentes (self):
        ...
        agentes = self._genericDataFrame.GetDataHdfs('AGENTES','file_AGENTE_CIRCUITO')
        agentes = agentes.filter(col('CLASE_UNEGOCIO_CODIGO')=='INT')\
        .select(col('EMPRESA_CODIGO').alias('agt_int_empresa_id_bk'),upper(col('EMPRESA_NOMBRE')).alias('agt_int_empresa'),
                col('REGION_CODIGO').alias('agt_int_region_id_bk'),upper(col('REGION_NOMBRE')).alias('agt_int_region'),
                col('UNEGOCIO_CODIGO').alias('agt_int_unegocio_id_bk'),upper(col('UNEGOCIO_NOMBRE')).alias('agt_int_unegocio'),
                col('ESTACION_CODIGO').alias('agt_int_linea_id_bk'),
                upper(col('ESTACION_NOMBRE')).alias('agt_int_linea'))
    ...
    def Transform_data (self, extract_data):
        ...
        maxPk = self._accesoDatos.GetMaxPkTable('SELECT COALESCE(MAX(agt_int_id_pk),0) pk FROM cen_dws.dim_agt_interconexion')   
        self.catalogoDW = self._accesoDatos.GetAllData('cen_dws.dim_agt_interconexion')
        fecha_carga = datetime.datetime.today()

        for toReplace, replacement in zip(paramReplace, valueReplace):
            extract_data = extract_data\
            .select(extract_data.agt_int_empresa_id_bk,
                    Funct.trim(regexp_replace(extract_data.agt_int_empresa, toReplace, replacement)).alias('agt_int_empresa'),
                    extract_data.agt_int_region_id_bk, 
                    extract_data.agt_int_region,
                    extract_data.agt_int_unegocio_id_bk, 
                    Funct.trim(regexp_replace(extract_data.agt_int_unegocio, toReplace, replacement)).alias('agt_int_unegocio'),
                    extract_data.agt_int_linea_id_bk, 
                    Funct.trim(regexp_replace(extract_data.agt_int_linea, toReplace, replacement)).alias('agt_int_linea'))
        ...
        if self.catalogoDW.count()==0: 
            catalogos = extract_data.select((maxPk+row_number().over(Window.partitionBy()\
                            .orderBy(extract_data.agt_int_empresa_id_bk,
                                     extract_data.agt_int_unegocio_id_bk,
                                     extract_data.agt_int_linea_id_bk))).alias('agt_int_id_pk'),'*', 
            ...
            data_to_compare = extract_data.join(self.catalogoDW,
                                                (extract_data.agt_int_empresa_id_bk==self.catalogoDW.agt_int_empresa_id_bk) &\
                                                (extract_data.agt_int_unegocio_id_bk==self.catalogoDW.agt_int_unegocio_id_bk) &\
                                                (extract_data.agt_int_linea_id_bk==self.catalogoDW.agt_int_linea_id_bk), how='left')\
                                        .select(Funct.when(self.catalogoDW.agt_int_id_pk.isNull(),0)\
                                                .otherwise(self.catalogoDW.agt_int_id_pk).alias("agt_int_id_pk"), 
                                                extract_data.agt_int_empresa_id_bk, extract_data.agt_int_empresa, 
                                                extract_data.agt_int_region_id_bk, extract_data.agt_int_region, 
                                                extract_data.agt_int_unegocio_id_bk, extract_data.agt_int_unegocio, 
                                                extract_data.agt_int_linea_id_bk, extract_data.agt_int_linea)                

            # Identificación de registros nuevos que deben ser insertados en el DWH
            dataNw = data_to_compare.filter(data_to_compare.agt_int_id_pk==0).drop('agt_int_id_pk')
            ...
            dataMdf = data_to_compare.filter(data_to_compare.agt_int_id_pk!=0)
    ...
    def Load_Upsert_data(self,transform_data):
        ...
        result_transact = []
        transform_data_map = transform_data.rdd.map(lambda x: (x.agt_int_id_pk, [x.agt_int_empresa_id_bk,x.agt_int_empresa,
                                                                                 x.agt_int_region_id_bk,x.agt_int_region,
                                                                                 x.agt_int_unegocio_id_bk,x.agt_int_unegocio,
                                                                                 x.agt_int_linea_id_bk,x.agt_int_linea,x.fecha_carga]))
        
        querys_data_insert = transform_data_map.map(lambda x: Queries.Upsert_Query_Dim_Agente_Interconexion(x))
