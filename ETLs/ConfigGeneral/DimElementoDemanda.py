...
class ETLElementosDemandaBL():
    """Lógica de negocio para el ingreso de datos en la dimensión de elementos de demanda"""
    ...
    def Elt_main(self):
        try:
            ...
            print('---- Proceso de ETL de Dimensiones de ELEMENTOS de DEMANDA ---- \n')
            print('DATAWAREHOUSE: dwh_sirio')
            print('DIMENSIÓN: dim_agt_elemento_demanda \n')
            ...
            self.Load_data(transform_data,'cen_dws.dim_agt_elemento_demanda')
    ...
    def Extract_data_agentes (self):
        ...
        posicion_unidad = self._genericDataFrame.GetDataHdfs('CFG_PosicionUnidad','file_CFG_PosicionUnidad')
        unidad = self._genericDataFrame.GetDataHdfs('CFG_Unidad','file_CFG_Unidad')
        posicion_conectada = self._genericDataFrame.GetDataHdfs('CFG_PosicionesConectadas','file_CFG_PosicionesConectadas')
        posicion = self._genericDataFrame.GetDataHdfs('CFG_Posicion','file_CFG_Posicion')
        
        posiciones = posicion_conectada\
        .join(posicion, posicion_conectada.IdPosicion==posicion.IdPosicion)\
        .select(posicion.Codigo.alias('eldem_elemento_id_bk'),
                upper(posicion.Nombre).alias('eldem_elemento'))
        
        unidades = posicion_unidad\
        .join(unidad, posicion_unidad.IdUnidad==unidad.IdUnidad)\
        .select(unidad.Codigo.alias('eldem_elemento_id_bk'),
                upper(unidad.Nombre).alias('eldem_elemento'))
        
        elementos = posiciones.union(unidades)
    ...
    def Transform_data (self, extract_data):
        ...
        maxPk = self._accesoDatos.GetMaxPkTable('SELECT COALESCE(MAX(eldem_id_pk),0) pk FROM cen_dws.dim_agt_elemento_demanda')   
        self.catalogoDW = self._accesoDatos.GetAllData('cen_dws.dim_agt_elemento_demanda')
        fecha_carga = datetime.datetime.today()

        for toReplace, replacement in zip(paramReplace, valueReplace):
            extract_data = extract_data\
            .select(extract_data.eldem_elemento_id_bk,
                    Funct.trim(regexp_replace(extract_data.eldem_elemento, toReplace, replacement)).alias('eldem_elemento'))
        ...
        if self.catalogoDW.count()==0: 
            catalogos = extract_data.select((maxPk+row_number().over(Window.partitionBy()\
                            .orderBy(extract_data.eldem_elemento_id_bk))).alias('eldem_id_pk'),'*', 
            ...
            data_to_compare = extract_data.join(self.catalogoDW,
                                                (extract_data.eldem_elemento_id_bk==self.catalogoDW.eldem_elemento_id_bk), how='left')\
                                        .select(Funct.when(self.catalogoDW.eldem_id_pk.isNull(),0)\
                                                .otherwise(self.catalogoDW.eldem_id_pk).alias("eldem_id_pk"), 
                                                extract_data.eldem_elemento_id_bk,extract_data.eldem_elemento)                

            # Identificación de registros nuevos que deben ser insertados en el DWH
            dataNw = data_to_compare.filter(data_to_compare.eldem_id_pk==0).drop('eldem_id_pk')

            catalogos = dataNw.select((maxPk+row_number().over(Window.partitionBy()\
                                                               .orderBy(dataNw.eldem_elemento_id_bk))).alias('eldem_id_pk'),
                                      '*',Funct.concat(Funct.lit(fecha_carga)).cast('timestamp').alias('fecha_carga'))

            # Identificación de registros que han sufrido cambios en la base transaccional y deben ser modificados en el DW
            dataMdf = data_to_compare.filter(data_to_compare.eldem_id_pk!=0)
    ...
    def Load_Upsert_data(self,transform_data):
        ...
        result_transact = []
        transform_data_map = transform_data.rdd.map(lambda x: (x.eldem_id_pk, [x.eldem_elemento_id_bk, x.eldem_elemento, x.fecha_carga]))
        
        querys_data_insert = transform_data_map.map(lambda x: Queries.Upsert_Query_Dim_Elemento_Demanda(x))
